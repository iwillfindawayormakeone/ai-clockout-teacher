# Ember & Stone Studio — handmade candles, gemstone jewelry, beads & soap

A six-page static website for a small "handmade ritual and gifting" brand. No build step: open `index.html` in a browser, or drop the folder on any static host (Netlify, Vercel, GitHub Pages).
...
6. **Photos.** Replace every illustration (marked "Illustration — real photo goes here") with real, unfiltered photos of the actual item, with something for scale. This is the single most important swap. The hero illustration is in `index.html`; product art is inside each `<article class="product">`.
